...
var checkIfCanBreak = function(s1, s2) {
    let sortedS1 = Array.from(s1).sort()
    let sortedS2 = Array.from(s2).sort()


    let [aseCount, desCount] = [0, 0]

    for(let i = 0;i<sortedS1.length;i++){
        if(sortedS1[i]<=sortedS2[i]) aseCount++;
        if(sortedS2[i]<=sortedS1[i]) desCount++;
    }

    return aseCount==sortedS1.length || desCount == sortedS1.length
};
// @lc code=end
