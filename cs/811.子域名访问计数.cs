...
public class Solution {
    public IList<string> SubdomainVisits(string[] cpdomains) {
        ...
        foreach(var temp in cpdomains) {
            var pairs = temp.Split(" ");
            var times = Int32.Parse(pairs[0]);
            var domain = pairs[1];
            var startIdx = 0;

            do{
                domain = domain.Substring(startIdx);
                if(!count.ContainsKey(domain)) {
                    count.Add(domain,times);
                }else{
                    count[domain]+=times;
                }
                startIdx = domain.IndexOf('.')+1;
            }while(!string.IsNullOrEmpty(domain) && startIdx>0); 
        }

        var res = new List<string>{};
        foreach(var key in count) {
            res.Add(string.Format("{0} {1}",key.Value,key.Key));
        }
