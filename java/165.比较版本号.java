...
class Solution {
    public int compareVersion(String version1, String version2) {
        ...
        var len = Math.min(version1Split.length, version2Split.length);
        for (int i = 0; i < len; i++) {
            var v1 = Integer.parseInt(version1Split[i]);
            var v2 = Integer.parseInt(version2Split[i]);
            if (v1 != v2)
                return v1 > v2 ? 1 : -1;
        }

        var left = version1Split.length > version2Split.length ? version1Split : version2Split;
        for (int i = len; i < left.length; i++) {
            if (Integer.parseInt(left[i]) != 0)
