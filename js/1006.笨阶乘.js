...
const cacheFirst = new Map()
const cacheClumsy = new Map()

const first = (N) => {
    if(!cacheFirst.has(N)) {
        if(N===1) cacheFirst.set(N,1)
        else if(N===2) cacheFirst.set(N, 2)
        // else if(N===3) return 6
        else cacheFirst.set(N, Math.floor(N * (N-1) / (N-2))) // + N - 3
    }
    return cacheFirst.get(N)
    
}

var clumsy = function (N) {
    if(!cacheClumsy.has(N)) {
        if(N<4) cacheClumsy.set(N, first(N))
        else cacheClumsy.set(N, first(N) + N - 3 - 2 * first(N-4) + clumsy(N-4))
    }

    return cacheClumsy.get(N)
